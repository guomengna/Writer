package com.example.guome.writer.Image;
/**
 * Created by guome on 2017/9/21.
 */
...
public class CircleImageViewByMyself extends ImageView {
    //缩放类型
    ...
    //构造函数
    public CircleImageViewByMyself(Context context) {
        super(context);
        init();
    }
    //构造函数
    public CircleImageViewByMyself(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }
    /**
     * 构造函数
     */
    public CircleImageViewByMyself(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CircleImageViewByMyself, defStyle, 0);
        //通过TypedArray提供的一系列方法getXXXX取得我们在xml里定义的参数值；
        // 获取边界的宽度
        mBorderWidth = a.getDimensionPixelSize(R.styleable.CircleImageViewByMyself_border_width, DEFAULT_BORDER_WIDTH);
        // 获取边界的颜色
        mBorderColor = a.getColor(R.styleable.CircleImageViewByMyself_border_color, DEFAULT_BORDER_COLOR);
        mBorderOverlay = a.getBoolean(R.styleable.CircleImageViewByMyself_border_overlay, DEFAULT_BORDER_OVERLAY);
...
//                    canvas.drawText(mFlagText, getWidth() / 2,(3 + (float)Math.ceil((float) (Math.PI * 5 / 18)))
...
//                new int[] { Color.rgb(255, 255, 255), Color.rgb(1, 209, 255) }, null);
